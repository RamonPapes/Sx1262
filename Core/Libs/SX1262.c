...
void SX1262_HandleCallback(uint8_t *buf ,uint8_t *len)
{
 ...
 //Clear all interrupt flags.  This should result in the interrupt pin going low
  ...
  while (HAL_GPIO_ReadPin(LORA_1262_DIO1_GPIO_Port, LORA_1262_DIO1_Pin))
	{
...
void SX1262_Radio_essental_Config(void)
{
  ...
  //is often called frequently when changing the radio config, it's broken up into its own function
		...
		cmnd[1] =  0xa;   //ModParam1 = Spreading Factor.  Can be SF5-SF12, written in hex (0x05-0x0C)
		cmnd[2] =  0x04;   //ModParam2 = Bandwidth.  See Datasheet 13.4.5.2 for details. 0x00=7.81khz (slowest)
		cmnd[3] =  0x04;   //ModParam3 = CodingRate.  Semtech recommends CR_4_5 (which is 0x01).  Options are 0x01-0x04, which correspond to coding rate 5-8 respectively
